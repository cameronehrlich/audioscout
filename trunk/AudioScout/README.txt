...
AudioData  c library for AudioScout example client program (for reading audio files)
   ...
   Compile the /libs folder and install the AudioData and pHashAudio libraries.
   Compile the bindings folder in /libs/bindings/java, and install the 
   pHashAudio-jni library.  Run the ant build script to build the pHashAudio jar.


mpg123     c library for reading mp3 files.
   Download and compile before AudioData libs, since AudioData will depend on this one.
