...
-----------------------------------------------------------------------
	...
	4. Start the auscoutd server with the address to find the metadatadb
	   ...
	   NOTE:  The port number must start on a port at which the next
           5 ports are open ports.  The server uses the ports in this range to 
           communicate with the table servers.
...
libAudioData.so - lib to read audio data
libpHashAudio.so - lib to compute hash values and indexing storage functions

audioindex - utility program to build index, query against the index, and other abilities.

auscoutd - program to run the central auscout server.
tblservd - program to run an instance of a table server.
metadatadb - program to run an instance of metadata database server.

audiodb.sql - database script to create the database.

auscoutclient - console-based client application to do lookups and insertions.

auscout-client.c - source code for the auscoutclient program.  This can serve as a demo
                   for how to write client applications to interface with the  auscoutd server.

auscout-gui   - shell script to run the client gui application

auscout       - gui qt-based program 
